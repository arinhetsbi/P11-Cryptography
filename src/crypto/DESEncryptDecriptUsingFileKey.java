...
public class DESEncryptDecriptUsingFileKey {
    ...
    public static byte[] Encriprt(String data, byte [] keyEncoded){
        byte[] encrypted = null;
        try {
    ...
    public static byte[] Descrypt(byte[] encrypted, byte[] keyEncoded ){
        byte[] decrypted = null;
        try {
            SecretKeySpec key = new SecretKeySpec(keyEncoded, "DES");
            Cipher cipher = Cipher.getInstance("DES/ECB/PKCS5Padding");
    ...
    public static void main(String[] args) {
        ...
        byte[] encrypted = Encriprt(message, key);
        byte[] descryted = Descrypt(encrypted, key);
        
        System.out.println("Plain Text: "+message);
        System.out.println("Encrypted: "+new String(encrypted));
        System.out.println("Encrypted Hex: "+
                MyStringUtils.getHexString(encrypted));
        System.out.println("Descrypted: "+new String(descryted));
    }
